...
class StatisticsEngine:
    ...
    def get_weekly_summary(self, date: datetime.datetime = None) -> Dict[str, float]:
        ...
        end_date = start_date + datetime.timedelta(days=7) - datetime.timedelta(seconds=1)
    ...
    def get_category_expenses(self, start_date: datetime.datetime = None,
    ...
    def get_top_expenses(self, limit: int = 3, start_date: datetime.datetime = None,
                        end_date: datetime.datetime = None) -> List[Tuple[str, float]]:
        """获取指定时间段内支出最高的分类
    ...
    def get_trend_data(self, period: str = "month", count: int = 6) -> List[Dict]:
        ...
        for i in range(count-1, -1, -1):
            if period == "day":
                ...
                period_str = f"{week_start.strftime('%m-%d')}~{end_date.strftime('%m-%d')}"
            else:  # month
                month = today.month - i
                year = today.year
                while month <= 0:
